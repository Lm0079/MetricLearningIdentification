...
#This class is used to evaluate model performance while Zebra is being actively used
class Zebra2(data.Dataset):
	# Class constructor
	def __init__(	self,
					fold,
					fold_file,
		...
		self.__root = "/user/work/gh18931/diss/datasets/grevvy_dataset/new_aug_4"
		...
		# The split we're after (e.g. train/validation)
		self.__split = split
		...
		self.__train_images_dir = os.path.join(self.__root, "train")
		self.__test_images_dir = os.path.join(self.__root, "test")
		#self.__validate_images_dir = os.path.join(self.__root, "validation")
		...
		#self.__validate_folders = allFoldersAtDir(self.__validate_images_dir)
		
		assert len(self.__train_folders) - 1 == len(self.__test_folders)
		#assert len(self.__train_folders) -1  == len(self.__validate_folders)
		...
		#validation_files = {os.path.basename(f):allFilesAtDirWithExt(f, ".png") for f in self.__validate_folders}
		...
		#self.__sorted_files['validation'] = {k:v for (k,v) in validation_files.items() if k not in remove}
		...
		#validation_list = [v for k,v in self.__sorted_files['validation'].items()]
		...
		#self.__files['validation'] = [item for sublist in validation_list for item in sublist]
	...
	def printStats(self):
		print("Loaded the Zebra dataset_____________________________")
